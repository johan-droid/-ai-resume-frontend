...
class _ResumeBuilderScreenState extends State<ResumeBuilderScreen> {
  ...
  final ScrollController _scrollController =
      ScrollController(); // Make sure this is here
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (_scrollController.hasClients) {
        ...
        _scrollController.animateTo(
          _scrollController
              .position.maxScrollExtent, // Scroll to the very bottom
  ...
  Future<void> _generateFullPdf() async {
    ...
    final String fullName = _resumeDetails['fullName'] ??
        _fullDriverResumeData['fullName'] ??
        'N/A';
    final String jobTitle = _resumeDetails['jobType'] ??
        _fullDriverResumeData['jobTitle'] ??
        'N/A'; // Use 'jobType' from chat
    final String phone = _resumeDetails['phone'] ??
        _fullDriverResumeData['contact']?['phone'] ??
        'N/A';
    final String location = _resumeDetails['city'] ??
        _fullDriverResumeData['contact']?['location'] ??
        'N/A';
    final String email =
        _fullDriverResumeData['contact']?['email'] ?? 'N/A'; // From dummy
    ...
    final List<String> skills = (_resumeDetails['skills'] != null &&
            _resumeDetails['skills']!.isNotEmpty)
        ? _resumeDetails['skills']!.split(',').map((s) => s.trim()).toList()
        : (_fullDriverResumeData['skills'] as List? ?? [])
            .map((s) => s.toString())
            .toList();
    ...
    final List experienceDetails =
        _fullDriverResumeData['experience'] as List? ?? [];

    // Qualification: Use chat input if available, otherwise dummy
    final String qualification = _resumeDetails['qualification'] ??
        _fullDriverResumeData['education']?['degree'] ??
        'N/A';
    final String qualificationInstitution =
        _fullDriverResumeData['education']?['institution'] ?? '';
    final String qualificationYear =
        _fullDriverResumeData['education']?['year'] ?? '';

    final List certifications =
        _fullDriverResumeData['certifications'] as List? ?? []; // From dummy
    final List languages =
        _fullDriverResumeData['languages'] as List? ?? []; // From dummy
        ...
        build: (pw.Context context) {
          return pw.Column(
            ...
            children: [
              ...
              pw.Text('$experienceYears years total',
                  style: pw.TextStyle(
                  ...
                  child: pw.Column(
                    ...
                    children: [
                      ...
                      ...expDetailsList.map((d) => pw.Padding(
                            padding:
                                const pw.EdgeInsets.only(left: 8.0, top: 2.0),
                            child: pw.Row(
                                crossAxisAlignment: pw.CrossAxisAlignment.start,
                                children: [
                                  pw.Text('• '),
                    ...
                    pw.Expanded(
                      child: pw.Column(
                          ...
                          children: [
                            ...
                            if (qualificationInstitution.isNotEmpty)
                              pw.Text(qualificationInstitution),
                            if (qualificationYear.isNotEmpty)
                              pw.Text('Year: $qualificationYear'),
                          ]),
                    ),
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return _buildChatMessage(
                  // Use your existing chat bubble widget
